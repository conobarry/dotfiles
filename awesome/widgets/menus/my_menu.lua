...
local function load_theme(a, b)
    ...
    fallback = fallback.menu or fallback
...
local function set_size(_menu)
    ...
    local in_dir, other = 0, 0
    local a, b = "height", "width"

    local direction = _menu.layout.dir or "y"

    if direction == "x" then  a, b = b, a  end
...
local function grabber(_menu, _, key, event)
    ...
    local sel = _menu.selected_index or 0
...
-- Called when a menu item is executed
...
-- Called whenever an item is focussed
-- @param num the index of the item
function menu:item_enter(index, opts)

    opts = opts or {}

    -- Get the item that the mouse has entered
    local item = self.items[index]

    if index == nil or self.selected_index == index or not item then
        return
    -- If an item is already selected, leave it
    elseif self.selected_index then
        self:item_leave(self.selected_index)
    end

    --print("selected", num, menu.selected_index, item.is_selectable, item.theme.bg_focus)

    if item.is_selectable == true then
...
function menu:is_under_mouse()
    ...
    local current_menu = self:get_root()

    while current_menu.active_child do
        if wibox_under_mouse == current_menu.wibox then
            return true
        end
        current_menu = current_menu.active_child
    end

    return false
    
    --return mouse.current_wibox
...
function menu:add(args, index)
    ...
    --print("item added "..inspect(args, {depth = 2}))
...
function menu:delete(num)
    ...
    if self.selected_index == num then
        self:item_leave(self.selected_index)
        self.selected_index = nil
    end
    self.layout:reset()
...
function menu.entry(args) -- luacheck: no unused args
    
    --print("menu.entry arg: \n"..inspect(args))
...
-- Lookup table for the types of menu entries
menu.entry_type = {
    text = menu.text_entry,
...
function menu.new(args, parent)
    ...
    local default_layout = wibox.widget {
        layout = wibox.layout.fixed.vertical,
        fill_space = false
    }
    --default_layout.fill_space = false

    args.layout = wibox.layout.flex.vertical-- or default_layout.layout

    local _menu = table_update(object(), {
        item_enter = menu.item_enter,
        item_leave = menu.item_leave,
        get_root = menu.get_root,
        is_under_mouse = menu.is_under_mouse,
        delete = menu.delete,
        update = menu.update,
        toggle = menu.toggle,
        hide = menu.hide,
    ...
    _menu.wibox:connect_signal("mouse::leave", 
        function()
            ...
            if not _menu.active_child then
                _menu:hide()
                if _menu.parent then
                    if not _menu:is_under_mouse() then
