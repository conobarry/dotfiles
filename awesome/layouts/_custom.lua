...
local function mouse_resize_handler(c, _, _, _, orientation)
  ...
  print("Workarea height: " .. workarea.height)

  if orientation == "tile" then

    cursor = "cross"

    if client_geom.height + 15 > workarea.height then
      offset = client_geom.height * .5
      cursor = "sb_h_double_arrow"

    elseif not (client_geom.y + client_geom.height + 15 > workarea.y + workarea.height) then
      offset = client_geom.height
    end

    corner_coords = {
      x = workarea.x + workarea.width * mwfact,
      y = client_geom.y + offset
    }
  ...
  capi.mousegrabber.run(function(_mouse)
    ...
    for _, v in ipairs(_mouse.buttons) do
      if v then
        ...
        local fact_x = (_mouse.x - workarea.x) / workarea.width
        local fact_y = (_mouse.y - workarea.y) / workarea.height
        local new_mwfact
        ...
        if (geom.y + geom.height + 15) > (workarea.y + workarea.height) then
          wfact_y = (geom.y + geom.height - _mouse.y) / workarea.height
        else
          wfact_y = (_mouse.y - geom.y) / workarea.height
        end

        if (geom.x + geom.width + 15) > (workarea.x + workarea.width) then
          wfact_x = (geom.x + geom.width - _mouse.x) / workarea.width
        else
          wfact_x = (_mouse.x - geom.x) / workarea.width
        end
...
local function tile_group(geometries, clients, workarea, orientation, fact, group, useless_gap)
  ...
  local available = workarea[width] - (group.coord - workarea[x])
  ...
  local coord = workarea[y]
  local used_size = 0
  local unused = workarea[height]
  for c = group.first, group.last do
    local geom = {}
    local hints = {}
    local i = c - group.first + 1
    ...
    geom[x] = group.coord
    geom[y] = coord
    geometries[clients[c]] = geom
    hints.width, hints.height = apply_size_hints(clients[c], geom.width,
...
local function do_tile(param, orientation)
  ...
  local selected_tag = param.tag or capi.screen[param.screen].selected_tag
  ...
  local num_masters = math.min(selected_tag.master_count, #clients)
  local num_slaves = math.max(#clients - num_masters, 0)

  local master_width_factor = selected_tag.master_width_factor
  local workarea = param.workarea
  local num_columns = selected_tag.column_count

  local data = tag.getdata(selected_tag).windowfact

  if not data then
    data = {}
    tag.getdata(selected_tag).windowfact = data
  end

  local coord = workarea[x]
  ...
  local grow_master = selected_tag.master_fill_policy == "expand"
  -- this was easier than writing functions because there is a lot of data we need
  for _ = 1, 2 do

    if place_master and num_masters > 0 then

      local size = workarea[width]

      if num_slaves > 0 or not grow_master then
        size = math.min(workarea[width] * master_width_factor, workarea[width] - (coord - workarea[x]))
      end

      if num_slaves == 0 and not grow_master then
        coord = coord + (workarea[width] - size) / 2
      end

      if not data[0] then data[0] = {} end

      coord = coord + tile_group(geometries, clients, workarea, orientation, data[0], {
        first = 1,
        last = num_masters,
        coord = coord,
        size = size
    ...
    if not place_master and num_slaves > 0 then
      local last = num_masters

      -- we have to modify the work area size to consider left and top views
      local wasize = workarea[width]
      if num_masters > 0 and (orientation == "left" or orientation == "top") then
        wasize = workarea[width] - workarea[width] * master_width_factor
      end
      for i = 1, num_columns do
        -- Try to get equal width among remaining columns
        local size = math.min((wasize - (coord - workarea[x])) /
                      (num_columns - i + 1))
        local first = last + 1
        last = last + math.floor((#clients - last) / (num_columns - i + 1))
        -- tile the column and update our current x coordinate
        if not data[i] then data[i] = {} end
        coord = coord + tile_group(geometries, clients, workarea, orientation, data[i], {
          first = first,
          last = last,
          coord = coord,
          size = size
...
-- function tile.left.arrange(p) return do_tile(p, "left") end
...
-- function tile.bottom.arrange(p) return do_tile(p, "bottom") end
...
-- function tile.top.arrange(p) return do_tile(p, "top") end
